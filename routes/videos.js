...
// Download video from Google Drive - Fixed gdown implementation
router.post('/download', async (req, res) => {
  try {
    ...
    if (!fileId) {
      return res.status(400).json({
        success: false,
        message: 'Invalid Google Drive URL. Please use a valid share link.'
      });
    }
    ...
    try {
      ...
      try {
        ...
        await execAsync('pip3 install gdown');
      }

      // Download using gdown
      await execAsync(`gdown --id ${fileId} -O "${outputPath}" --no-check-certificate`);
